...
!  type(snadata) :: sd(sn_num)
  ...
  real(kind=4) :: sd(3,sn_num)
  common sd
  character(80) :: filename = "sn_z_mu_dmu.txt"
  logical :: alive
  character(6) :: sn_name
  integer :: i
  real(kind=4),external :: chi2
!  real(kind=4) :: best
  real(kind=4) :: LnLike(n+1),Like(n+1)
...
implicit none
  ...
  real(kind=4) :: chi2, chi2sn,chi2R,chi2A,R,A
  real(kind=4) :: omegam0,abar,bbar,cbar
  real(kind=4),external :: muth
...
!instead of calculating it everytime when call the function chi2sn
  ...
  R=sqrt(omegam0)*Dl(omegam0,1089.0)

  chi2R = ((R-1.715)**2)/(0.021**2)
  
  A=sqrt(omegam0)*((Dl(omegam0,0.35)/0.35)**(2.0/3.0))/(hubble(omegam0,0.35)**(1.0/3.0))
