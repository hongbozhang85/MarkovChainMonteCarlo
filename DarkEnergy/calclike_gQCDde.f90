...
subroutine read_sn_data
  ...
  common sd
  character(80) :: filename = "sn557hubble12.txt"
  logical :: alive
  character(6) :: sn_name
  integer :: i
  character(255) :: hbversion = "SN+BAO+HUB+CMB | newdata | GENERALIZED_QCD_DE 4pars"

!---read sna data---!
  write(*,*) 'Reading Supernovea Data, Union II & 12 Hubble Data'
...
!  integer,parameter :: num_params=3 !P = [omegam,h100,omegab,beta]
...
!WMAP7
  data((cov(rr,cc),rr=1,3),cc=1,3)/ 2.305,29.698,-1.333,29.698,6825.270,-113.180,-1.333,-113.180,3.414 /
...
! WMAP7
  cmbarray(1)= la-302.09
  cmbarray(2)= R-1.725
  cmbarray(3)= zs-1091.3
...
real(kind=8),parameter :: z3=1.0, a=-1.0

! P = [omegam,h100,omegab,beta]

  myhubble=0.5*(1-P(1))*((1+z)**(P(4)/2.0))+sqrt(P(1)*((1+z)**3)+4.15E-5*((1+z)**4)/(P(2)*P(2))+0.25*((1+z)**P(4))*(1-P(1))*(1-P(1)))
