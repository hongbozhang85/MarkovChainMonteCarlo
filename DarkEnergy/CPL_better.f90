...
!---best fit---!
  ...
  beginomega = 0.276374505888725
  beginw = -1.09381921310541
  beginwa = 0.641708029535039
...
!instead of calculating it everytime when call the function chi2sn
  ...
  R=sqrt(omegam0)*RDl(omegam0,w,wa,1.089D3)
...
function RDl(omegam0,w,wa,z)   ! /int_{0}^{z} dt/H(t) lum-distance=(1+z)Dl
! this part is to do trape method of integration
! read notes/../integration/trape.f90 for detail of this method
implicit none

  real(kind=8) :: RDl,omegam0,w,wa,z
  real(kind=8),external :: hubble
  real(kind=8) :: xmin=0.0,xmax   
  integer,parameter :: xmid=5.0,seg= 10000, seg1=500
  ...
  do i=0,seg1,1
     datalist1(i+1)=1.0/hubble(omegam0,w,wa,xmin+i*width1)
  end do

  do i=0,seg,1
     datalist(i+1)=1.0/hubble(omegam0,w,wa,xmid+i*width)
  end do
