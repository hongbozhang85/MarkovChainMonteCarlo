! 0911130 by hongbo
! 397SNa[Constitution] + R + A /  SuQP's Model, 2 Bins with z2 fixed to 1 & a fixed to -1
...
!  type(snadata) :: sd(sn_num)
  ...
  integer,parameter :: sn_num = 397, n=100
  real(kind=8) :: sd(3,sn_num)
  common sd
  character(80) :: filename = "sn397.dat"
  logical :: alive
  character(6) :: sn_name
...
! beginx = [omegam0,z1,a0,a1,a2,a=-1]
...
!--------------!
  ...
  beginx(1) = 0.286693626340366  !!!0.2
  beginx(2) = 0.311159557620783  !!!0.4
  beginx(3) = -0.522591656365250  !!!-0.6
  beginx(4) = -1.53318273977436  !!!-1.5
  beginx(5) = 0.825101223737931  !!!0.5
  beginx(6) = -1.0
...
!instead of calculating it everytime when call the function chi2sn
  ...
  R=sqrt(param(1))*RDl(param,1.089D3)
...
function RDl(param,z)   ! /int_{0}^{z} dt/H(t) lum-distance=(1+z)Dl
implicit none

  real(kind=8) :: RDl,z,param(6)
  real(kind=8),external :: hubble
  real(kind=8) :: xmin=0.0,xmax 
  ...
  do i=0,seg1,1
     datalist1(i+1)=1.0/hubble(param,xmin+i*width1)
  end do

  do i=0,seg,1
     datalist(i+1)=1.0/hubble(param,xmid+i*width)
  end do
